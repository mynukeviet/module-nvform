...
if( $nv_Request->isset_request( 'export', 'post, get' ) )
{
	...
	// Set properties
	$objPHPExcel->getProperties()->setCreator( $admin_info['username'] )
	...
	// Set font size
	$objPHPExcel->getActiveSheet()->getStyle( "A1:" . $highestColumn . $highestRow )->getFont()->setSize( 13 );
...
else
{
	...
	foreach( $array_type as $key => $value )
	{
		...
		$xtpl->assign( 'TYPE', array( 'key' => $key, 'value' => $value, 'checked' => $ck ) );
